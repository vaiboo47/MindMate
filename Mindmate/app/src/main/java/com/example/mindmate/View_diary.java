    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        fab.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Intent ij=new Intent(getApplicationContext(), add_diary.class);
                startActivity(ij);
            }
        });

        sh = PreferenceManager.getDefaultSharedPreferences(getApplicationContext());
        url = sh.getString("url", "") + "/stud_view_diary";
                        ...
                        try {
                            ...
                            if (jsonObj.getString("status").equalsIgnoreCase("ok")) {
                                ...
                                for(int i=0;i<js.length();i++)
                                {
                                    ...
                                    comp[i]=u.getString("cont");
                                    date[i]=u.getString("date");
                                    time[i]=u.getString("time");
                                    emo[i]=u.getString("emo");



                                }
                                L1.setAdapter(new custom_diary(getApplicationContext(),id, date, time, comp, emo));//custom_view_service.xml and li is the listview object
